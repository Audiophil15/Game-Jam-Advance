...
func _process(_delta):
	if self.modulate == Color(1,1,1,1) and showlabels and labeltoshow<4:
		label = get_node("Label%d"%labeltoshow)
		label.set_bbcode("[center][fade start="+str(i)+" length=10]" + labels[labeltoshow] + "[/fade][/center]")
		i+=0.5
...
func hide() :
	...
	showlabels = 0
	for i in range(4) :
		get_node("Label%d"%i).set_bbcode("")

func enableText(number) :
	get_node("Label%d"%number).visible = true
